...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$bitcoin_qt_want_version" = "xauto" && test x$bitcoin_qt_force != xyes; then
    if test x$bitcoin_enable_qt != xno; then
      AC_MSG_WARN([$1; fint-qt frontend will not be built])
    fi
...
dnl This must be called before any other BITCOIN_QT* mfintos to ensure that
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build fint-qt GUI (default=auto, qt5 tried first)])],
    [
...
dnl All mfintos below are internal and should _not_ be used from the main
dnl configure.ac.
dnl ----
