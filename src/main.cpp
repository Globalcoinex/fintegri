...
#if defined(NDEBUG)
#error "Fint cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 80000000 * COIN;
	        ...
	        nSubsidy = 1 * COIN;
	} else if (nHeight >= 500 && nHeight <= 904320 ) {
	        nSubsidy = 1 * COIN;
	...
	} else if (nHeight <= 350600 && nHeight >= 2977924) {
	        nSubsidy = 0.03125 * COIN;	
	} else if (nHeight <= 4029124 && nHeight >= 350661) {
	        nSubsidy = 0.03125 * COIN;	
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < 701) {
        ret = blockValue * 0;
    } else if(nHeight <= 350660 && nHeight >= 350601){
    	ret = blockValue * 0.999999;
    } else {
        ret = blockValue * 0.8;
    }
...
static CBlockIndex* FindMostWorkChain()
{
    do {
        ...
        while (pindexTest && !chainActive.Contains(pindexTest)) {
            ...
            // for the most work chain if we come fintoss them; we can't switch
