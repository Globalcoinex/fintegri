...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FINT);
    ...
    if (!settings.contains("nAnonymizeFintAmount"))
        settings.setValue("nAnonymizeFintAmount", 1000);

    nPrivateSendRounds = settings.value("nPrivateSendRounds").toLongLong();
    nAnonymizeFintAmount = settings.value("nAnonymizeFintAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeFintAmount"))
        SoftSetArg("-anonymizefintamount", settings.value("nAnonymizeFintAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fint.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFintAmount:
            return QVariant(nAnonymizeFintAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case PrivateSendRounds:
            ...
            break;
        case AnonymizeFintAmount:
            nAnonymizeFintAmount = value.toInt();
            settings.setValue("nAnonymizeFintAmount", nAnonymizeFintAmount);
            emit anonymizeFintAmountChanged(nAnonymizeFintAmount);
            break;
        case CoinControlFeatures:
