...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::FINT)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("FINT")
    updateDisplayUnit();
}
...
void OverviewPage::updatePrivateSendProgress()
{
    ...
    QString strAnonymizeFintAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeFintAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeFintAmount = strAnonymizeFintAmount.remove(strAnonymizeFintAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFintAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeFintAmount * COIN) nMaxToAnonymize = nAnonymizeFintAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeFintAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeFintAmount));
        strAnonymizeFintAmount = strAnonymizeFintAmount.remove(strAnonymizeFintAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFintAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    } else {
